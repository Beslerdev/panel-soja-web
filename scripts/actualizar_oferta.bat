...
echo ============================================
echo   Actualizando solapa Oferta del panel (DS HNOS)
echo ============================================
...
rem --- Archivo Excel: el que se arrastro sobre este .bat, o "ventas.xlsx" en esta carpeta ---
set "EXCEL_FILE=%~1"
if "%EXCEL_FILE%"=="" (
    if exist "ventas.xlsx" set "EXCEL_FILE=ventas.xlsx"
)
if "%EXCEL_FILE%"=="" (
    for /f "delims=" %%f in ('dir /b /o-d "*.xlsx" 2^>nul') do (
        if "%EXCEL_FILE%"=="" set "EXCEL_FILE=%%f"
    )
)
if "%EXCEL_FILE%"=="" (
    echo ERROR: no encontre ningun archivo .xlsx.
    echo Arrastra el ventas.xlsx actualizado sobre este .bat, o dejalo en esta carpeta.
    pause
    exit /b 1
)
...
"%NODE_CMD%" importar_oferta.js "%EXCEL_FILE%"
if errorlevel 1 (
    echo.
