...
function transformRows(results) {
  ...
  const lotes = results.lotes.map((r) => ({
    ...
    // Datos de laboratorio del lote. El panel los muestra en la solapa
    // Despachos, al hacer clic en el ojo de cada linea (busca por lote_final).
    pg: r.pg,
    pg_curado: r.pg_curado,
    p1000: r.p1000,
  }));
  data.lotes = lotes;
  ...
  // -------- Oferta --------
  // Hoja "ventas.xlsx" del panel de Oferta (proyecto separado, "Proyecto
  // consultas" / ventas-dashboard) — cuánto hay ofrecido, vendido y
  // disponible por variedad, en bolsas ("Primu"). Se suma acá como una
  // solapa más para tener todo en un solo panel.
  const oferta = (results.oferta_variedades || []).map((r) => ({
    semillero: r.semillero,
    varied: r.varied,
    variedad: r.variedad,
    oferta: r.oferta_primu || 0,
    venta: r.venta_primu || 0,
    disponible: r.disponible_primu || 0,
  }));
  data.oferta = oferta;
  const totalOferta = sum(oferta, (o) => o.oferta);
  const totalVentaOferta = sum(oferta, (o) => o.venta);
  const totalDisponible = sum(oferta, (o) => o.disponible);
  data.kpi_oferta = {
    total_oferta: totalOferta,
    total_venta: totalVentaOferta,
    total_disponible: totalDisponible,
    pct_vendido: totalOferta ? (totalVentaOferta / totalOferta) * 100 : 0,
    variedades: oferta.length,
  };
